    ...
    <main className="min-h-screen" style={{ background: "#fafafa" }}>
      ...
      <div className="px-6 pt-12 pb-5 border-b" style={{ background: "#fff8f2", borderColor: "#f8d7e6" }}>
        <div className="flex items-center gap-3 max-w-md mx-auto">
          <Link href="/dashboard" className="text-lg" style={{ color: "#cdb4db" }}>←</Link>
      ...
      <div className="px-5 py-6 flex flex-col gap-4 max-w-md mx-auto">
        ...
        <div className="rounded-3xl p-5" style={{ background: "#fff8f2", border: "1.5px solid #cdb4db" }}>
          <p className="text-xs mb-3" style={{ color: "#b79bcf" }}>🌍 Sprache</p>
        ...
        {/* Benutzername */}
        <form onSubmit={saveProfile} className="rounded-3xl p-5 flex flex-col gap-3" style={{ background: "#fff8f2", border: "1.5px solid #f8d7e6" }}>
          <p className="text-xs" style={{ color: "#b79bcf" }}>👤 Benutzername & E-Mail</p>
          ...
          <button type="submit" className="w-full text-white font-medium rounded-2xl py-3 text-sm hover:opacity-90 transition-opacity"
            style={{ background: profileSaved ? "#cfe8d5" : "#b79bcf" }}>
            {profileSaved ? "✓ Gespeichert" : "Speichern"}
          </button>
        </form>

        {/* Passwort */}
        <form onSubmit={savePassword} className="rounded-3xl p-5 flex flex-col gap-3" style={{ background: "#fff8f2", border: "1.5px solid #f8d7e6" }}>
          <p className="text-xs" style={{ color: "#b79bcf" }}>🔒 Passwort ändern</p>
          ...
          {pwError && <p className="text-xs" style={{ color: "#b79bcf" }}>{pwError}</p>}
          <button type="submit" className="w-full text-white font-medium rounded-2xl py-3 text-sm hover:opacity-90 transition-opacity"
            style={{ background: pwSaved ? "#cfe8d5" : "#b79bcf" }}>
        ...
        <div className="rounded-3xl p-5 flex flex-col gap-2" style={{ background: "#fff8f2", border: "1.5px solid #ffd9c7" }}>
          <p className="text-xs mb-1" style={{ color: "#b79bcf" }}>🔑 Konto</p>
          ...
          <Link href="/" className="w-full text-center font-medium rounded-2xl py-3 text-sm hover:opacity-90 transition-opacity"
            style={{ background: "#fafafa", border: "1.5px solid #f8d7e6", color: "#cdb4db" }}>
            Abmelden
          </Link>
        </div>

        <p className="text-center text-xs pb-4" style={{ color: "#cdb4db" }}>Luma 🌸</p>
      </div>
    </main>
  );
}
